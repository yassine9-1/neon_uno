...
*A massive co-located multiplayer card game — smartphones as controllers, projector as the arena.*
...
3. Generate self-signed SSL certificates (**mandatory step** — mobile browsers require HTTPS to access the accelerometer):
   ...
   node gen-cert.js
   ```
   *This creates `cert.pem` and `key.pem` files at the project root.*
...
The game is accessible at **`https://localhost:3000`**. The local network IP is displayed in the console so other devices can connect via QR Code.
